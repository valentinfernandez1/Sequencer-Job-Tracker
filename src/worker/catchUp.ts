...
export async function catchUp(provider: JsonRpcProvider): Promise<number>{
    ...
    while(currentBlock<latestBlock) {
        ...
        const amountBlocks = latestBlock - currentBlock > batchPullAmount ? batchPullAmount : latestBlock - currentBlock;

        const eta = estimateCatchUpTime(currentBlock,latestBlock,startTime,startBlock);

        console.log(`Catching Up (${currentBlock}/${latestBlock}) - Pulling ${amountBlocks} Blocks - ETA: ${eta} secs`);
        
        const blocks = await bulkPullBlocks(provider, currentBlock, amountBlocks)

        console.log(`Blocks (${currentBlock}/${currentBlock + amountBlocks}) Received - Inspecting for Job transactions`);
...
async function bulkPullBlocks(provider: JsonRpcProvider, fromBlock: number, amountBlocks: number): Promise<( Block | null )[]> {
...
async function bulkInspectBlock(provider: JsonRpcProvider, blocks: ( Block | null )[]) {
    ...
    for (const block of blocks) {
        if (!block) return;
        inspectPromises.push(findJobInBlock(provider, block).then((maybeJob) => maybeJob? handleFoundJob(maybeJob) : null));
    }
